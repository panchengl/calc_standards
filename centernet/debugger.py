...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    # elif num_classes == 6 or dataset == 'dianli':
    elif num_classes == cfg.class_num or dataset == 'dianli':
      self.names = dianli_class_name
  ...
  def show_all_imgs(self, pause=False, show=True, time=0):
    global j
    if not self.ipynb:
      for i, v in self.imgs.items():
        if show:
            cv2.namedWindow('{}'.format(i), 0)
            cv2.resizeWindow('{}'.format(i), 1200, 800);
        ...
        print("i is", i)
        j += 1
        # cv2.imwrite("results_centernet/" + str(j) + '.jpg', v)
...
# dianli_class_name =  ["DiaoChe", "TaDiao", "TuiTuJi", "BengChe", "WaJueJi", "ChanChe"]
dianli_class_name =  ["DiaoChe", "TaDiao", "ShiGongJiXie", "YanHuo", "SuLiaoBu"]
# dianli_class_name =  ["DiaoChe", "TaDiao", "ShiGongJiXie"]
color_list = np.array(
        [
            1.000, 1.000, 1.000,
            0.850, 0.325, 0.098,
