...
def create_directory(create_dir):
    if not os.path.exists(create_dir):
        ...
        print("directions: %s  create success"%create_dir)
        return True
    print("directions: %s  is exit, do not create"%create_dir)
    return True
...
def postprocess_doctor_yang(pred_bbox, test_input_size, org_img_shape, conf_thres = 0.3, nms_th=0.5):
    ...
    bboxes = nms(bboxes, conf_thres, nms_th, method='nms')
    return bboxes
...
def read_class_names_tf(class_name_path):
    names = {}
    with open(class_name_path, 'r') as data:
        for ID, name in enumerate(data):
            names[ID] = name.strip('\n')
